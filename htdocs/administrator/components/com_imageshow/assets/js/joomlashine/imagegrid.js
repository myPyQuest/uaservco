		...
		*/
		...
		ImageGrid.initEvents      = function(){
				...
				ImageGrid.jsnjTree = ImageGrid.contents.categories.jsnjtree(jsnjTreeOptions).bind('jsn_jtree.selectitem', function(e, obj){
					...
					if ( obj.attr('checked') == 'checked' ){
						...
						$.post( baseUrl+'administrator/index.php?option=com_imageshow&controller=images&task=savesync', {
							...
							ImageGrid.showlistImages.html('<div class="showlist-sync-item-notice jsn-bglabel"><span class="jsn-icon64 icon-refresh"></span>'+JSNISLang.translate('SHOWLIST_NOTICE_IMAGES_ARE_SYNCED')+'</div>');
						});
					}else{
						...
						$.post( baseUrl+'administrator/index.php?option=com_imageshow&controller=images&task=removesync', {
							...
							if(!catselected){
								ImageGrid.showlistImages.html('<div class="showlist-sync-item-notice jsn-bglabel"><span class="jsn-icon64 icon-refresh"></span>'+JSNISLang.translate('SHOWLIST_NOTICE_IN_SYNC_MODE')+'</div>');
							}
						});
					}
				});
			...
			$('a', ImageGrid.sourcePanelHeader).unbind("click").click(function(){
				if ( $(this).hasClass('media-show-grid') && !$(this).hasClass('active') ){
				...
				}else if($(this).hasClass('media-show-list') && !$(this).hasClass('active')){
					$(this).addClass('active');
			...
			$('button', ImageGrid.treePanelHeader).unbind("click").click(function(){
				...
				if ( $(this).hasClass('expand-all') ){
					...
					if ( $(this).hasClass('btn-success') ){
						...
						ImageGrid.showlistImages.html('<div class="jsn-bglabel showlist-drag-drop-item-notice"><span class="jsn-icon64 icon-pointer"></span>'+JSNISLang.translate('SHOWLIST_NOTICE_DRAG_AND_DROP')+'</div>');
						//Save showlist
						ImageGrid.saveShowlist();
		...
		ImageGrid.RestoreShowlistShowType = function() {
			...
			if (!countShowlistImages)
			{
				ImageGrid.showlistPanelHeader.children('a.media-show-grid').removeClass('active').addClass('disabled').unbind("click");
				ImageGrid.showlistPanelHeader.children('a.media-show-list').removeClass('active').addClass('disabled').unbind("click");
			}
			else
			{
				ImageGrid.showlistPanelHeader.children('a.media-show-grid').removeClass('disabled').bind("click");
				ImageGrid.showlistPanelHeader.children('a.media-show-list').removeClass('disabled').bind("click");
				if (ImageGrid.cookie.exists('jsn-is-cookie-view-mode-imageshow-showlist') && ImageGrid.cookie.get('jsn-is-cookie-view-mode-imageshow-showlist')  == 'true'){
					ImageGrid.showlistPanelHeader.children('a.media-show-grid').removeClass('active');
					ImageGrid.showlistPanelHeader.children('a.media-show-list').addClass('active');
				}
				else
				{
					ImageGrid.showlistPanelHeader.children('a.media-show-grid').addClass('active');
					ImageGrid.showlistPanelHeader.children('a.media-show-list').removeClass('active');
				}
			}
		};
		/**
		...
		ImageGrid.RestoreSourceImagesShowType = function() {
			...
			{
				ImageGrid.sourcePanelHeader.children('a.media-show-grid').removeClass('active').addClass('disabled').unbind("click");
				ImageGrid.sourcePanelHeader.children('a.media-show-list').removeClass('active').addClass('disabled').unbind("click");
			}
			else
			{
				ImageGrid.sourcePanelHeader.children('a.media-show-grid').removeClass('disabled').bind("click");
				ImageGrid.sourcePanelHeader.children('a.media-show-list').removeClass('disabled').bind("click");
				if (ImageGrid.cookie.exists('jsn-is-cookie-view-mode-image-source') && ImageGrid.cookie.get('jsn-is-cookie-view-mode-image-source') == 'true' ){
					ImageGrid.sourcePanelHeader.children('a.media-show-grid').removeClass('active');
					ImageGrid.sourcePanelHeader.children('a.media-show-list').addClass('active');
				}
				else
				{
					ImageGrid.sourcePanelHeader.children('a.media-show-grid').addClass('active');
					ImageGrid.sourcePanelHeader.children('a.media-show-list').removeClass('active');
				}
			}
		};
		/**
		...
		ImageGrid.sortable        = function(){
			ImageGrid.sourceImages.sortable({
				...
				over : function(event, ui){
					if ( $(this).hasClass('showlist-items') && ImageGrid.showlistImages.find('div.media-item').length == 1 && ImageGrid.showlistImages.find('div.ui-sortable-placeholder').length ){
						ImageGrid.showlistImages.find('div.showlist-drag-drop-item-notice').remove();
					}
				},
				out : function(event, ui){

					if ( ImageGrid.showlistImages.find('div.media-item').length == 0 && ImageGrid.showlistImages.find('div.showlist-drag-drop-item-notice').length == 0){
						ImageGrid.showlistImages.html('<div class="jsn-bglabel showlist-drag-drop-item-notice"><span class="jsn-icon64 icon-pointer"></span>'+JSNISLang.translate('SHOWLIST_NOTICE_DRAG_AND_DROP')+'</div>');
					}					
				},
			...
			ImageGrid.showlistImages.sortable({
				...
				over: function (){
					$('.showlist-drag-drop-item-notice').hide();
					ImageGrid.showlistImages.removeClass('jsn-section-empty');
				},
				out: function (){
...
//			});
			...
			ImageGrid.showlistImages.children().each(function(){
				if ( $(this).hasClass('media-item') ){
					...
					if (ImageGrid.options.selectMode != 'sync'){
						$(this).children('div.item-index').append('<button class="delete-item pull-right jsn-icon16 icon-trash"></button><button class="edit-item pull-right jsn-icon16 icon-pencil"></button>');
...
//					//ImageGrid.saveShowlist();
		...
		ImageGrid.reindexImageSource = function(countImages){
			var btn = '';
			if(ImageGrid.options.selectMode!="sync")
				btn = '<button class="move-to-showlist pull-right jsn-icon16 icon-ok">&nbsp;</button>';
			var i = 1;
