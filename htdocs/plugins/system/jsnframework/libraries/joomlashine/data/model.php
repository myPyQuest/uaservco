 ...
 * @since    1.0.0
 */
class JSNDataModel extends JModel
{
	/**
	...
	public function backup($options = array())
	{
		...
		$this->data->addAttribute('extension-name',    'JSN ' . preg_replace('/JSN\s*/i', '', $info->name));
		...
		{
			JSNUtilsFile::forceDownload($name['zip'], $config->get('tmp_path') . DS . $name['zip'], 'application/zip', true);
		}
		else
		{
			return $config->get('tmp_path') . DS . $name['zip'];
		}
	}
	...
	protected function storeTableData($table, $rows)
	{
		...
		foreach ($rows AS $row)
		{
			...
			{
				$rowNode->addChild($name, htmlentities($value, ENT_QUOTES, 'UTF-8', false));
			}
		}
	}
	...
	protected function backupFiles($folders)
	{
		...
		foreach ($folders AS $folder => $filter)
		{
			...
			if ($files = JFolder::files(realpath(JPATH_ROOT . DS . $folder), $filter, true, true))
			{
				foreach ($files AS $file)
				{
					// Append to files array
					$this->files[] = array(
						'name' => trim(str_replace(realpath(JPATH_ROOT), '', realpath($file)), '/\\'),
	...
	protected function finalizeBackup($name)
	{
		...
		array_unshift($this->files, array('name' => $name['xml'], 'data' => $data));

		// Create zip file containing all backed up data and/or files
		$zip = new JArchiveZip;
		$zip = $zip->create($config->get('tmp_path') . DS . $name['zip'], $this->files);
	...
	public function restore($backup)
	{
		...
		if (is_array($backup))
		{
			if ( ! JFile::upload($backup['tmp_name'], $config->get('tmp_path') . DS . $backup['name']))
			{
				throw new Exception(JText::_('JSN_EXTFW_GENERAL_MOVE_UPLOAD_FILE_FAIL'));
			}
			$backup = $config->get('tmp_path') . DS . $backup['name'];
		}
		elseif ( ! preg_match('/^(\/|[a-z]:)/i', $backup))
		{
			$backup = JPATH_ROOT . DS . $backup;
		}

		// Do any preparation needed before doing real data restore
		try
		{
			$this->beforeRestore($backup);
		}
		catch (Exception $e)
		{
			throw $e;
	 ...
	 * @return  void
	 */
	protected function beforeRestore(&$backup)
	{
		// Initialize variables
		$com	= preg_replace('/^com_/i', '', JFactory::getApplication()->input->getCmd('option'));
		$info	= JSNUtilsXml::loadManifestCache();
		...
		$this->data = JSNUtilsXml::load($backup . DS . "jsn_{$com}_backup_db.xml");
		...
		if ( (string) $this->data['extension-name'] != 'JSN ' . preg_replace('/JSN\s*/i', '', $info->name))
		{
			throw new Exception(JText::_('JSN_EXTFW_DATA_INVALID_PRODUCT'));
		}
		elseif (isset($this->data['extension-edition'])
		...
		elseif ( ! version_compare($info->version, (string) $this->data['extension-version'], 'ge'))
		{
			...
			throw new Exception(
				...
				. '&nbsp;<a href="' . $ulink . '" class="jsn-link-action">' . JText::_('JSN_EXTFW_DATA_UPDATE_PRODUCT_NOW') . '</a>'
			);
		}
	}
	...
	protected function restoreFiles(&$backup)
	{
		...
		if ($this->files = JFolder::files($backup, '.', true, true))
		{
			...
			{
				if (basename($file) != $xml)
				{
					// Initialize file path
					$file = realpath($file);

					// Generate destination path
					$path = str_replace(realpath($backup), JPATH_ROOT, dirname($file));
	...
	protected function installSampleStep1()
	{
		...
		try
		{
			$path = $config->get('tmp_path') . DS . basename($url);
	...
	protected function installSampleStep2()
	{
		...
		{
			if ( ! JFile::upload($_FILES['sampleDataPackage']['tmp_name'], $config->get('tmp_path') . DS . $_FILES['sampleDataPackage']['name']))
			{
				throw new Exception(JText::_('JSN_EXTFW_GENERAL_MOVE_UPLOAD_FILE_FAIL'));
			}
			$path = $config->get('tmp_path') . DS . $_FILES['sampleDataPackage']['name'];
		}
		else
		{
