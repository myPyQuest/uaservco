<?php
/**
 * @version     $Id: asset.php 16453 2012-09-26 09:01:19Z binhpt $
...
abstract class JSNHtmlAsset
{
	...
	{
		$document = JFactory::getDocument();
		$document->addStyleDeclaration(trim($content), $type, $media, $attributes);
	}
	...
	private static function _getExtensionVersion ($extension)
	{
		...
		{
			// Get manifest cache
			$manifest = JSNUtilsXml::loadManifestCache($extension);

			// Store extension version
			self::$_loadedVersions[$extension] = isset($manifest->version) ? $manifest->version : '1.0.0';
		}
