<?php
/**
 * @version     $Id: generator.php 15965 2012-09-11 08:53:31Z cuongnm $
...
class JSNItemlistGenerator
{
	...
	*/
	public function JSNItemlistGenerator(JModelList $model)
	{
		$this->_model = $model;
		...
		$this->_component = JRequest::getCmd('option');
	...
	public function generateFilter()
	{
		...
		$this->_filterForm->loadFile(JPATH_COMPONENT_ADMINISTRATOR . DS . 'views' . DS . $this->_view . DS . 'filter.xml', true, '/filter');
	...
	public function getColumnHeader($column)
	{
		...
		$class		= isset($column['options']['classHeader']) ? ' class="center ' . $classDefault . $column['options']['classHeader'] . '"' : 'class="center ' . $classDefault . '"';
		...
		else
		{
			if ($type == 'checkbox' && $checAll)
			{
				...
				$html .= "<input type=\"checkbox\" name=\"toggle\" value=\"\" onclick=\"checkAll({$countItems});\" />";
			}
			else
			{
	...
	public function columnTypeLink($options)
	{
		...
		$link = preg_replace('/\{\$([^\}]+)\}/ie', '@$options["items"]->\\1', $link);
