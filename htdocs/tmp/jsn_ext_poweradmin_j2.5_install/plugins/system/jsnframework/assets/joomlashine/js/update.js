/**
 * Declare JSN Update Javascript library.
 *
 * @author     JoomlaShine.com
 * @copyright  JoomlaShine.com
 * @link       http://joomlashine.com/
 * @package    JSN Framework
 * @subpackage JSN Update
 * @version    $Id: update.js 16228 2012-09-21 02:41:34Z cuongnm $
...
function ($, modal)
{
	...
	JSNUpdate.prototype = {
		...
		step2: function() {
			...
			$.ajax({
				url: this.button.href + '&' + this.elements[this.current].getAttribute('ref'),
				...
				if (data.substr(0, 4) == 'DONE') {
					// Update indicators
					$('.jsn-update-downloading-indicator', this.elements[this.current]).removeClass('icon-loading').addClass('icon-ok');
					...
					// Update indicators
					$('.jsn-update-downloading-indicator', this.elements[this.current]).removeClass('icon-loading').addClass('icon-remove');
		...
		step3: function() {
			...
			this.modal = new modal({
	            ...
	            loaded: $.proxy(function() {
	    			...
	    			if (data.substr(0, 4) == 'DONE') {
	    				// Update indicators
	    				$('.jsn-update-installing-indicator', this.elements[this.current]).removeClass('icon-loading').addClass('icon-ok');
	    				...
	    				// Update indicators
	    				$('.jsn-update-installing-indicator', this.elements[this.current]).removeClass('icon-loading').addClass('icon-remove');
	    			...
	    			if (typeof this.elements[this.current] != 'undefined') {
	    				...
	    				this.button.href = this.button.href.replace('.install', '.download');
	    				this.button.data = '';
