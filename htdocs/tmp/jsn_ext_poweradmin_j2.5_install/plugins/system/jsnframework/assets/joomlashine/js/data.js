/**
 * Declare JSN Data Javascript library.
 *
 ...
 * @subpackage JSN Data
 * @version    $Id: data.js 16228 2012-09-21 02:41:34Z cuongnm $
	...
	JSNData.prototype = {
		...
		step1: function() {
			...
			.done(function(data) {
				...
				if (data.substr(0, 4) == 'DONE') {
					// Update indicators
					$('#jsn-data-install-sample-downloading-indicator').removeClass('icon-loading').addClass('icon-ok');
					...
					// Update indicators
					$('#jsn-data-install-sample-downloading-indicator').removeClass('icon-loading').addClass('icon-remove');
		...
		step2: function(manualInstall) {
			...
			if (manualInstall) {
				...
				$('#jsn-data-install-sample-indicator').show();
				$('#jsn-data-install-sample-downloading-indicator').removeClass('icon-remove').addClass('icon-ok');
				$('#jsn-data-install-sample-downloading-unsuccessful-message').hide();
				...
				this.iframe.attr('id', 'jsn-data-install-sample-manually').attr('_title', $('title').text()).load($.proxy(
					function(event) {
		            	var data = event.target.contentDocument.doctype == null ? event.target.contentDocument.body.innerHTML : 'DONE';
		    			this.finalize(data);
					},
					this
		...
		finalize: function(data) {
			...
			if (data.substr(0, 4) == 'DONE') {
				// Update indicators
				$('#jsn-data-install-sample-installing-indicator').removeClass('icon-loading').addClass('icon-ok');

				// State that installation is completed successfully
				$('#jsn-data-install-sample-successfully').show();
			} else {
				// Update indicators
				$('#jsn-data-install-sample-installing-indicator').removeClass('icon-loading').addClass('icon-remove');
